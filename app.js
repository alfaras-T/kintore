...
const PART_JA = Object.fromEntries(PARTS.map(p => [p.id, p.ja]));
...
function allExercises(){
  ...
  for (const s of sortedSets()) if (!seen.has(s.ex)){ seen.add(s.ex); out.push({ name:s.ex, part:s.part }); }
  return out;
}
...
function volumeByPart(fromDate){
  ...
  for (const s of db.sets) if (!fromDate || s.date >= fromDate) m[s.part] = (m[s.part] || 0) + vol(s);
  return m;
}
...
function suggest(){
  ...
  if (last) return { w:w2s(last.w), r:String(last.r) };
  const p = db.sets.filter(s => s.part === ui.part).sort((a,b) => b.ts - a.ts)[0];
  return p ? { w:w2s(p.w), r:String(p.r) } : { w:'20', r:'10' };
}
function syncInputs(){ const s = suggest(); ui.w = s.w; ui.r = s.r; }
...
function kpisHTML(){
  ...
  const v  = ss.reduce((a,s) => a + vol(s), 0);
  const ex = new Set(ss.map(s => s.ex)).size;
  const reps = ss.reduce((a,s) => a + s.r, 0);
  return `<div class="kpis" id="logKpis">
    <div class="kpi ${ss.length ? '' : 'kpi--dim'}"><span class="kpi-v n">${ss.length}</span><span class="kpi-l">セット</span></div>
    <div class="kpi ${reps ? '' : 'kpi--dim'}"><span class="kpi-v n">${reps}</span><span class="kpi-l">レップ</span></div>
    <div class="kpi ${v ? '' : 'kpi--dim'}"><span class="kpi-v n">${nf(v)}<small>kg</small></span><span class="kpi-l">総挙上量</span></div>
    <div class="kpi kpi--dim"><span class="kpi-v n">${ex}</span><span class="kpi-l">種目</span></div>
  </div>`;
}
...
function partsHTML(){
  ...
  return `<div class="parts">` + PARTS.map(p => {
    const c = ss.filter(s => s.part === p.id).length;
...
function recentsInner(){
  return exList(ui.part).slice(0, 5).map(n =>
...
function pickMenuHTML(){
  ...
  let head, items;
  if (q){
    items = all.filter(e => e.name.toLowerCase().includes(lc));
    head  = items.length ? '履歴から' : '新しい種目';
  } else {
    items = all.filter(e => e.part === ui.part);
  ...
  if (!rows.length) return `<div class="pm-sec">${
    all.length ? '該当する履歴がありません' : '種目名を入力してください'}</div>
    <div class="pm-hint">記録した名前がここに残り、次からは履歴から選べます。</div>`;
...
function prevHintHTML(){
  ...
  return `<div class="prev-hint" id="prevHint">
    <span class="hint-ex">${esc(ui.ex)}　<b>${done.length + 1}</b>セット目</span><br>
    ${ref ? `${when} <b>${ref.w ? w2s(ref.w)+'kg × '+ref.r : '自重 × '+ref.r}</b>
...
function ledgerHTML(){
  ...
  if (!ss.length) return `<div class="empty">
      ...
      <p>部位を選び、種目名を入力して、重量と回数を記録します。一度記録した種目名は履歴に残り、次からは入力欄の候補に出ます。</p>
    </div>`;
  ...
  return order.map(name => {
    ...
    return `<div class="lg-group" style="--pig:var(--p-${part})">
      <div class="lg-head"><span class="pig"></span><h3>${esc(name)}</h3>
        <span class="lg-meta">${rows.length}セット${v ? ` · <b>${nf(v)}</b>kg` : ''}</span></div>
      <ol>${rows.map((s, i) => {
        const isPR = pr && pr.id === s.id && setsOfEx(name).length > 1;
        return `<li class="lg-row" data-id="${s.id}">
          <span class="lg-i">${i+1}</span>
...
function sideTodayHTML(){
  ...
  const by = {}; for (const s of ss) by[s.part] = (by[s.part] || 0) + Math.max(vol(s), s.r * 8);
  const tot = Object.values(by).reduce((a,b) => a+b, 0) || 1;
  const used = PARTS.filter(p => by[p.id]);
  return `<div class="split">${used.map(p =>
      `<i style="--pig:var(--p-${p.id}); flex:${(by[p.id]/tot).toFixed(4)}"></i>`).join('')}</div>` +
    used.map(p => {
      const rows = ss.filter(s => s.part === p.id);
      const v = rows.reduce((a,s) => a + vol(s), 0);
      const r = rows.reduce((a,s) => a + s.r, 0);
      return `<div class="mini-row" style="--pig:var(--p-${p.id})"><span class="pig"></span>
        <span>${p.ja}</span><b>${v ? nf(v)+'<i>kg</i>' : r+'<i>回</i>'}</b></div>`;
    }).join('');
}
...
function refreshLog(){
  ...
  $('#btnLog').innerHTML = (ui.editing ? '更新する' : 'セットを記録') + '<kbd>⏎</kbd>';
  renderRail();
}
...
function editSet(id){
  ...
  ui.editing = id; ui.ex = s.ex; ui.part = s.part;
  ui.w = w2s(s.w); ui.r = String(s.r);
  $('#exIn').value = s.ex; $('#inW').value = ui.w; $('#inR').value = ui.r;
  refreshLog();
...
function dupSet(id){
  const s = db.sets.find(x => x.id === id); if (!s) return;
  ui.ex = s.ex; ui.part = s.part; ui.w = w2s(s.w); ui.r = String(s.r);
  db.sets.push({ ...s, id:uid(), ts:Date.now() }); save();
  $('#exIn').value = s.ex; refreshLog();
  if (db.settings.auto) tStart(db.settings.rest);
}
...
$('#page-log').addEventListener('click', e => {
  ...
  else if (l === 'applyPrev'){ setW(w2s(+b.dataset.w)); setR(b.dataset.r); }
...
$('#page-log').addEventListener('keydown', e => {
  ...
  if (e.key === 'Enter' && (t.id === 'inW' || t.id === 'inR')){ e.preventDefault(); logSet(); }
  if (t.id === 'noteIn' && e.key === 'Enter') t.blur();
  if ((t.id === 'inW' || t.id === 'inR') && (e.key === 'ArrowUp' || e.key === 'ArrowDown')){
    e.preventDefault();
    const up = e.key === 'ArrowUp';
    if (t.id === 'inW') setW(w2s(clamp((parseFloat(ui.w)||0) + (up ? 1 : -1) * db.settings.wStep, 0, 999)));
    else setR(String(clamp((parseInt(ui.r,10)||0) + (up ? 1 : -1), 0, 999)));
  }
});
...
const effVol = s => Math.max(vol(s), s.r * 8);
...
function calHTML(){
  ...
  const totalS = inMonth.reduce((a,d) => a + setsOn(d).length, 0);
  ...
  const cellHTML = d => {
    ...
    const by = {}; for (const s of ss) by[s.part] = (by[s.part] || 0) + effVol(s);
    const bars = PARTS.filter(p => by[p.id]).map(p =>
...
function calSideHTML(){
  ...
  return head + `
    <div class="cd-kpis">
      <div class="kpi"><span class="kpi-v n">${ss.length}</span><span class="kpi-l">セット</span></div>
    ...
    ${order.map(name => {
      const rows = ss.filter(s => s.ex === name);
      return `<div class="cd-ex" style="--pig:var(--p-${rows[0].part})">
        <div class="cd-ex-t"><span class="pig"></span><h4>${esc(name)}</h4>
          <b>${nf(rows.reduce((a,s)=>a+vol(s),0))}<i>kg</i></b></div>
        <div class="cd-sets">${rows.map(s => s.w ? `${w2s(s.w)}×${s.r}` : `${s.r}回`).join('<s>·</s>')}</div>
      </div>`;
    }).join('')}`;
}
...
function sparkHTML(name, part){
  const byDate = {};
  for (const s of setsOfEx(name)) byDate[s.date] = Math.max(byDate[s.date] || 0, s.w || s.r);
...
function exRowHTML(name, part){
  ...
  return `<div class="ex-row" style="--pig:var(--p-${part})">
    <div class="ex-main">
      ...
      <div class="ex-meta"><b>${ago}</b> · ${dates.length}日 ${rows.length}セット<button
        class="ex-ren" data-x="ren" data-v="${esc(name)}">名前を変更</button></div>
    </div>
    <div class="ex-pr n">${pr.w ? `${w2s(pr.w)}<i>kg</i> × ${pr.r}<em>推定1RM ${Math.round(est1RM(pr))}kg</em>`
                                : `${pr.r}<i>回</i><em>自重</em>`}</div>
    ${sparkHTML(name, part)}
...
function weekVolume(w){
  ...
  const by = Object.fromEntries(PARTS.map(p => [p.id, 0]));
  let tot = 0;
  for (const s of db.sets) if (s.date >= w && s.date <= end){ by[s.part] += effVol(s); tot += vol(s); }
  return { by, tot, eff:Object.values(by).reduce((a,b)=>a+b,0) };
}
...
function statHTML(){
  ...
  const setBy30 = Object.fromEntries(PARTS.map(p => [p.id,
    db.sets.filter(s => s.date >= from30 && s.part === p.id).length]));

  const prs = allExercises().map(e => ({ ...e, pr:prOf(e.name) }))
    ...
    <section class="stat-sec">
      ...
      <div class="wchart">
        ${ws.map((x, i) => {
          ...
          const segs = PARTS.filter(p => x.by[p.id]).map(p =>
          ...
          return `<div class="wcol ${i === ws.length-1 ? 'is-cur' : ''}">
            <span class="wcol-v n">${nf(x.tot)}kg</span>
